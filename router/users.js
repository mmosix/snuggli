...
router.get('/profile', verifyToken, (req, res) => {
    ...
    jsonwebtoken.verify(req.token, privateKey, (err, authorizedData) => {
        if (err) {
            ...
            db.query('SELECT u.* , "" as password, s.school_name FROM users u LEFT JOIN schools s ON u.school_id = s.school_id WHERE u.id LIKE ?', userId, function (error, results, fields) {
                if (error) throw error;
                return res.status(200).send({
                    error: false,
                    data: results[0],
